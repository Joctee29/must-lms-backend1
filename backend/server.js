...
console.log("🟢 Pool config created (backend). Using DATABASE_URL?", !!process.env.DATABASE_URL);
const pool = new Pool(poolConfig);

pool.on('connect', (client) => {
  console.log('🟢 New DB client connected! (backend)');
});

pool.on('error', (err, client) => {
  console.error('❌ Unexpected DB client error (backend):', err.message);
});

pool.on('acquire', (client) => {
  console.log('📥 DB client acquired from pool (backend)');
});

pool.on('release', (client) => {
  console.log('📤 DB client released back to pool (backend)');
});
...
const queryWithRetry = async (text, params, retries = 5) => {
  ...
  while (attempts < retries) {
    try {
      console.log(`🔍 Running query (attempt ${attempts+1}/${retries}) (backend)`);
      return await pool.query(text, params);
    } catch (err) {
      attempts++;
      console.error(`❌ Query failed (attempt ${attempts}/${retries}) (backend):`, err.message);
      if (attempts >= retries) {
        throw err;
      }
      // Wait before retrying (2 seconds per attempt)
      console.log(`⏳ Waiting ${attempts * 2000}ms before retry... (backend)`);
